<?php
    ...
    $managerName = trim(strtolower($_GET["ManagerName"]));

    //If member name isn't empty
    if ($managerName != "")
    {   
        //Build query based on their first name
        $findUser = "SELECT FirstName, LastName FROM Users WHERE FirstName LIKE '".$db->real_escape_string($managerName)."%' AND managerID IS NOT NULL";
